    ...
    : ITaskItemRepository
{
    ...
    public Task<TaskItem?> GetByIdReadOnlyAsync(
        Guid taskItemId,
        CancellationToken cancellationToken = default)
    {
        return _dbContext.TaskItems
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        {
            query = query.Where(
                taskItem => taskItem.Status == options.Status.Value);
        }

        if (options.Priority.HasValue)
        {
            query = query.Where(
                taskItem => taskItem.Priority == options.Priority.Value);
        }

        if (options.AssigneeId.HasValue)
        {
            query = query.Where(
                taskItem => taskItem.AssigneeId == options.AssigneeId.Value);
        }

        if (options.DueFromUtc.HasValue)
        {
            query = query.Where(
                taskItem => taskItem.DueDateUtc.HasValue &&
                    taskItem.DueDateUtc.Value >= options.DueFromUtc.Value);
        }
        ...
        if (options.DueToUtc.HasValue)
        {
            query = query.Where(
                taskItem => taskItem.DueDateUtc.HasValue &&
                    taskItem.DueDateUtc.Value <= options.DueToUtc.Value);
        }
        ...
        if (!string.IsNullOrWhiteSpace(options.Search))
        {
            ...
            query = query.Where(
                taskItem =>
                    EF.Functions.ILike(taskItem.Title, pattern) ||
                    (taskItem.Description != null &&
                     EF.Functions.ILike(taskItem.Description, pattern)));
        }
        ...
        SortDirection sortDirection)
    {
        return (sortBy, sortDirection) switch
        {
            (TaskItemSortBy.CreatedAt, SortDirection.Asc) =>
                query.OrderBy(taskItem => taskItem.CreatedAtUtc)
                    .ThenBy(taskItem => taskItem.Id),
            (TaskItemSortBy.CreatedAt, SortDirection.Desc) =>
                query.OrderByDescending(taskItem => taskItem.CreatedAtUtc)
                    .ThenBy(taskItem => taskItem.Id),
            (TaskItemSortBy.DueDate, SortDirection.Asc) =>
                query.OrderBy(taskItem => taskItem.DueDateUtc == null)
            ...
            (TaskItemSortBy.Priority, SortDirection.Asc) =>
                query.OrderBy(taskItem => taskItem.Priority)
                    .ThenBy(taskItem => taskItem.Id),
            (TaskItemSortBy.Priority, SortDirection.Desc) =>
                query.OrderByDescending(taskItem => taskItem.Priority)
                    .ThenBy(taskItem => taskItem.Id),
            (TaskItemSortBy.Status, SortDirection.Asc) =>
                query.OrderBy(taskItem => taskItem.Status)
                    .ThenBy(taskItem => taskItem.Id),
            (TaskItemSortBy.Status, SortDirection.Desc) =>
                query.OrderByDescending(taskItem => taskItem.Status)
                    .ThenBy(taskItem => taskItem.Id),
            (TaskItemSortBy.Title, SortDirection.Asc) =>
                query.OrderBy(taskItem => taskItem.Title)
                    .ThenBy(taskItem => taskItem.Id),
            (TaskItemSortBy.Title, SortDirection.Desc) =>
                query.OrderByDescending(taskItem => taskItem.Title)
                    .ThenBy(taskItem => taskItem.Id),
            (TaskItemSortBy.UpdatedAt, SortDirection.Asc) =>
                query.OrderBy(taskItem => taskItem.UpdatedAtUtc ?? taskItem.CreatedAtUtc)
                    .ThenBy(taskItem => taskItem.Id),
            (TaskItemSortBy.UpdatedAt, SortDirection.Desc) =>
                query.OrderByDescending(taskItem => taskItem.UpdatedAtUtc ?? taskItem.CreatedAtUtc)
                    .ThenBy(taskItem => taskItem.Id),
            _ => query.OrderBy(taskItem => taskItem.CreatedAtUtc)
                .ThenBy(taskItem => taskItem.Id)
        };
    }
}
